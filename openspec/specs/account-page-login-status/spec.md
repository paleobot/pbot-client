# account-page-login-status

Login-status header on the Profile and My Account pages, sourced from `useAuth()` plus a username cached at login time. The header tells the user whether they are signed in and, when known, under what username.
