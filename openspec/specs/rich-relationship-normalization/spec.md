# rich-relationship-normalization Specification

## Purpose

Define how the PBOT client normalizes "rich relationship" fields returned by the GraphQL API so that phantom rows (rows whose inner target entity is `null`) are filtered out of list-valued fields and coerced to `null` on singular fields, consistently across every results surface.
