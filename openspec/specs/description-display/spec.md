...
## Purpose
Defines how an individual description renders on the Specimen and OTU detail pages — the header label (description name plus a schema link) and the inline references subsection — across the web and PDF views, while leaving the OTU merged-exemplar rendering unchanged.
